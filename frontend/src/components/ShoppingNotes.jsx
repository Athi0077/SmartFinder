...
const ShoppingNotes = ({ isOpen, onClose }) => {
 ...
 const navigate = useNavigate();
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-gray-50 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-l border-gray-200 flex flex-col">
      <div className="p-4 border-b border-gray-200 flex items-center justify-between bg-white">
        <h2 className="text-xl font-bold text-gray-800">Your Cart & Bill</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-3 bg-primary-50 border-b border-primary-100 flex justify-between items-center">
            <div className="flex items-center space-x-2 text-primary-800 font-bold">
              <ShoppingCart className="h-5 w-5" />
              <span>Shopping Cart ({cartItems.length})</span>
            </div>
            {cartItems.length > 0 && (
              <button onClick={clearCart} className="text-xs text-primary-600 font-semibold hover:text-primary-800">Clear</button>
            )}
          </div>
          ...
          <div className="p-2">
            {cartItems.length === 0 ? (
              ...
              <ul className="space-y-2">
                {cartItems.map(item => (
                  <li key={item.id} className="flex flex-col p-2 bg-gray-50 rounded-lg border border-gray-100">
                    <div className="flex justify-between items-start mb-1">
                      <span className="font-semibold text-gray-800 text-sm">{item.product?.name}</span>
                      <span className="font-bold text-primary-600 text-sm">${((item.product?.price || 0) * (item.quantity || 1)).toFixed(2)}</span>
                    </div>
                    
                    <div className="flex items-center space-x-3 mb-2">
                      <div className="flex items-center space-x-2 bg-white rounded border border-gray-200">
                        <button onClick={() => updateQuantity(item.id, -1)} className="px-2 text-gray-500 hover:bg-gray-100 rounded-l">-</button>
                        <span className="text-xs font-bold text-gray-700 w-4 text-center">{item.quantity || 1}</span>
                        <button onClick={() => updateQuantity(item.id, 1)} className="px-2 text-gray-500 hover:bg-gray-100 rounded-r">+</button>
                      </div>
                      <span className="text-xs text-gray-500">${item.product?.price?.toFixed(2) || '0.00'} each</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-1 space-x-2">
                      ...
                      <div className="flex space-x-2 flex-1 justify-end">
                        <button
                          ...
                          className="flex items-center space-x-1 bg-primary-50 text-primary-700 hover:bg-primary-100 px-2 py-1.5 rounded-lg font-medium transition-colors text-xs"
                        >
                        ...
                        <button
                          ...
                          className="flex items-center space-x-1 bg-primary-600 text-white text-xs px-2 py-1.5 rounded-lg hover:bg-primary-700 font-medium shadow-sm transition-colors"
                        >
                          <span>To Bill</span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-2">
            {billItems.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">No items added to bill yet.</p>
            ) : (
              <ul className="space-y-2">
                {billItems.map(item => (
                  <li key={item.id} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg border border-gray-100">
                    <div className="flex flex-col">
                      <span className="font-semibold text-gray-800 text-sm line-clamp-1">{item.product?.name} <span className="text-gray-500 font-normal ml-1">x{item.quantity || 1}</span></span>
                      <button onClick={() => deleteItem(item.id)} className="text-xs text-red-500 text-left hover:underline mt-1">Remove</button>
                    </div>
                    <span className="font-bold text-gray-900 text-sm">${((item.product?.price || 0) * (item.quantity || 1)).toFixed(2)}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <div className="p-4 bg-gray-100 border-t border-gray-200 flex justify-between items-center">
            <span className="text-sm font-bold text-gray-600 uppercase tracking-wider">Total Amount</span>
            <span className="text-xl font-black text-gray-900">${billTotal.toFixed(2)}</span>
          </div>
        </div>
